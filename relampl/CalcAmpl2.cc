...
bool buildDecayTopologyRelAmpl(const Setting* parent, int p_J = 0, int p_P = 0) {
	...
	if (isobar) {
		const Setting* isobar0 = findLibConfigGroupAtIndex(*isobar,0,false);
		if (isobar0) {
			if (not buildDecayTopologyRelAmpl(isobar0))
				return false;
		}
		
		const Setting* isobar1 = findLibConfigGroupAtIndex(*isobar,1,false);
		if (isobar1){
			if (not buildDecayTopologyRelAmpl(isobar1))
				return false;
		}
	}
	...
	if (fsParticles) {
		const Setting* fsParticles0 = findLibConfigGroupAtIndex(*fsParticles,0,false);
		if (fsParticles0) {
		...
		const Setting* fsParticles1 = findLibConfigGroupAtIndex(*fsParticles,1,false);
		if (fsParticles1) {
		...
		//		const Setting* fsParticles2 = findLibConfigGroupAtIndex(*fsParticles,2);
		//		if (fsParticles2) {
