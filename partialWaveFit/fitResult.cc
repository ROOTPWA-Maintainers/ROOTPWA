                ...
                const bool                      hasHessian)
{
	...
	if ( (fitParCovMatrix!=nullptr or fitParCovMatrixIndices!=nullptr) and
	     (fitParCovMatrix==nullptr or fitParCovMatrixIndices==nullptr) ){
		printErr << "Got only one of fitParCovMatrix or fitParCovMatrixIndices when filling a fitResult."
		         << "Need none or both arguments!" << endl;
		throw;
	}
	...
	if (fitParCovMatrix != nullptr) {
		...
		_fitParCovMatrix.ResizeTo(0, 0);
		_covMatrixValid = false;
		_hasHessian         = false; // set to falses, does not matter what is given externally
	}
...
fitResult::fill(const fitResult& result,
                ...
                const vector<double>*           phaseSpaceIntegral)      // normalization integral over full phase space without acceptance
{
	std::vector<std::complex<double>> prodAmpsEmpty;
	fill(result.nmbEvents(), result.normNmbEvents(), result.multibinBoundaries(), result.logLikelihood(),
	     result.rank(), prodAmpsEmpty, result.prodAmpNames(),
	     (fitParCovMatrix!=nullptr)? fitParCovMatrix : &result.fitParCovMatrix(),
	     (fitParCovMatrixIndices!=nullptr)? fitParCovMatrixIndices : &result.fitParCovIndices(),
	     (normIntegral!=nullptr)? normIntegral: &result.normIntegralMatrix(),
	     (acceptedNormIntegral!=nullptr)? acceptedNormIntegral : &result.acceptedNormIntegralMatrix(),
	     (phaseSpaceIntegral!=nullptr)? phaseSpaceIntegral: &result.phaseSpaceIntegralVector(),
	     result.converged(), result.hasHessian());
	// set prodAmps directly to avoid converting vector<TComplex> -> vector<comple<double>> -> vector<TComplex>
	_prodAmps               = result.prodAmps();
}


void
fitResult::fill(const fitResult& result, const bool fillCovMatrix, const bool fillIntegralMatrices)
{
	std::vector<std::complex<double>> prodAmpsEmpty;
	fill(result.nmbEvents(), result.normNmbEvents(), result.multibinBoundaries(), result.logLikelihood(),
	     result.rank(), prodAmpsEmpty, result.prodAmpNames(),
	     (fillCovMatrix)? &result.fitParCovMatrix() : nullptr,
	     (fillCovMatrix)? &result.fitParCovIndices(): nullptr,
	     (fillIntegralMatrices)? &result.normIntegralMatrix() : nullptr,
	     (fillIntegralMatrices)? &result.acceptedNormIntegralMatrix() : nullptr,
	     (fillIntegralMatrices)? &result.phaseSpaceIntegralVector() : nullptr,
	     result.converged(), result.hasHessian());
	// set prodAmps directly to avoid converting vector<TComplex> -> vector<comple<double>> -> vector<TComplex>
	_prodAmps               = result.prodAmps();
}
...
rpwa::getFitResultsFromFilesInMultibins(
	...
	for (const auto& fileName : fileNames) {
		...
		tree->SetBranchAddress(branchName.c_str(), inputFitResult.get());
