...
typedef complex<complexStruct<float >, float > floatStructComplex;
typedef complex<complexStruct<double>, double> doubleStructComplex;
...
#define TEXTURE_MEM_KERNEL(kernel, elementType)				\
  {									\
    kernel<elementType, elementType ## TextureReader><<< nmbBlocks, nmbThreadsPerBlock >>>((elementType*)deviceOutData[0], nmbElementsPerThread); \
  }
...
#define BENCHMARK(memoryType, kernel, elementType, nmbTransfersPerElement) \
  {									\
    ...
    cout << #kernel << "<" << #elementType << "> processed " << dataSize / (1024 * 1024) << " MiBytes in " \
	 << runTime * 1000 << " msec; total throughput: " << bandwidth / (1024 * 1024 * 1024) << " GiByte/sec" << endl; \
...
int main()
{
  ...
  cout << "allocating 4 * " << sizeof(float4) * nmbElements / (1024. * 1024.) << " MiBytes in global memory "
  ...
  // bind textures
  cutilSafeCall(cudaBindTexture(0, floatTexture,  deviceInData[0], sizeof(float ) * nmbElements));
