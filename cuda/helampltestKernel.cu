...
/// ////////////////////////////////////////////// ///
/// Blatt-Weisskopf barrier factor (not templated) ///
/// ////////////////////////////////////////////// ///
// note that for speed up purposes the barrier factor is defined without the squareroot here!
__device__ double
...
{
  double Pr = 0.1973;
  double z = (q * q)/(Pr * Pr);			// substitution
...
template<class T>
__device__ T
GammaFuncT( T m, T m0, T Gamma0, T q, T q0, int L) 
{
...
//  T Gamma = Gamma0 * (m0 / *m) * (*q / q0) * (BFValue / BF0) * (BFValue / BF0) ;
 double Gamma = ( Gamma0 * m0 * q  * BFValue ) / ( m * q0  * BF0 ) ; 	/// nearly 2 times (1.88) faster than the formula above, means first calculating the nominator and denominator and then dividing
 return Gamma;
}
  ...
  for(int lambda1 = -J1; lambda1 <= +J1; lambda1+=2) {
    for(int lambda2 = -J2; lambda2 <= +J2; lambda2+=2) {
      amplsum += 	 ::sqrt(L1+1.0) * cucgc(J1,lambda1,J2,lambda2,J1,lambda1-lambda2) * cucgc(L1,0,S1,lambda1-lambda2,JX,lambda1-lambda2) * cuDfuncConj(JX,MX,lambda1,theta1,phi1,0) * ::sqrt((double)BFactor(L1,qX)) * 
			 ::sqrt(L2+1.0) * cucgc(0,0,0,0,0,0) * cucgc(L2,0,0,0,L2,0) * cuDfuncConj(4,lambda1,0,theta2,phi2,0) * ::sqrt((double)BFactor(L2,qf2)) * 1.0; //bwig(wf2,1.2754,Gamma0,qf2,q0,L2);
    }
  }
  */
  return amplsum;
