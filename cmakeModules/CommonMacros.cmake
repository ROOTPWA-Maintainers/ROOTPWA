...
# helper function that parses version string from given pkg-config's .rc file
# this done using the CMake facilities
# an alternative approach would be to use the output of the pkg-config executable
function(parse_version_from_pkg_config_file RC_FILE VERSION)
	set(${VERSION} "NOTFOUND" PARENT_SCOPE)
	if(NOT EXISTS "${RC_FILE}")
		message(WARNING "pkg-config file '${RC_FILE}' does not exist. Cannot parse version.")
	else()
		file(STRINGS ${RC_FILE} _VERSION_LINE REGEX "Version:")
		list(LENGTH _VERSION_LINE _VERSION_LINES)
		if(_VERSION_LINES EQUAL 1)
			string(REGEX MATCHALL "[0-9]+" _VERSION_COMPONENTS ${_VERSION_LINE})
			...
			message(WARNING "Error while parsing version string from '${RC_FILE}': "
				"got ${_VERSION_LINES} 'Version:' lines instead of 1.")
		endif()
		unset(_VERSION_LINE)
