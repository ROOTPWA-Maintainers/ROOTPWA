  ...
  for (unsigned int itree = 0; itree < 2; ++itree) {
    ...
    for (unsigned int i = 0; i < nevt; ++i) {
	 ...
	 if (percent % 10 == 0)
      ...
      std::vector<std::pair<std::pair<float, float>, double> > comb;
      for(unsigned int i=0; i < npart; i++) {
        for(unsigned int j=0; j < i; j++) {
          TLorentzVector lv = event.getParticle(i).p() + event.getParticle(j).p();
          double mass2 = lv.M2();
			// coding also the mass to the charge in order to distinguish K-pi+ and pi-pi+
          std::pair<float, float> charge(event.getParticle(i).q()*nt.getParticle(i).p().M(), event.getParticle(j).q()*event.getParticle(j).p().M());
          std::pair<std::pair<float, float>, double> temp;
