...
TH1D* hMIsobar2APS=new TH1D("hMIsobar2APS"+massbin,"Isobar mass (Accepted PS)",nbninsm,0.4,2.7);

 TH1D* hMIsobar3Data=new TH1D("hMIsobar3Data"+massbin,"Isobar mass (DATA)",nbninsm,0.2,2.3);
 hMIsobar3.push_back(hMIsobar3Data);
TH1D* hMIsobar3APS=new TH1D("hMIsobar3APS"+massbin,"Isobar mass (Accepted PS)",nbninsm,0.2,2.3);
 ...
 TH1D* hHe22PhiAPS=new TH1D("hHe22PhiAPS"+massbin,"Helicity Phi 2pi2pi (Accepted PS)",nbinsang,-TMath::Pi(),TMath::Pi());


 TH1D* hHe31Data=new TH1D("hHe31ThData"+massbin,"Helicity Theta 3pi1pi (Data)",nbinsang,-1,1);
    hHe31.push_back(hHe31Data);
 TH1D* hHe31APS=new TH1D("hHe31ThAPS"+massbin,"Helicity Theta 3pi1pi (Accepted PS)",nbinsang,-1,1);
 ...
 TH1D* hHe31PhiAPS=new TH1D("hHe31PhiAPS"+massbin,"Helicity Phi 3pi1pi (Accepted PS)",nbinsang,-TMath::Pi(),TMath::Pi());


 TH1D* hHe21Data=new TH1D("hHe21ThData"+massbin,"Helicity Theta 2pi1pi (Data)",nbinsang,-1,1);
    hHe21.push_back(hHe21Data);
 TH1D* hHe21APS=new TH1D("hHe21ThAPS"+massbin,"Helicity Theta 2pi1pi (Accepted PS)",nbinsang,-1,1);
 ...
 TH1D* hHe21PhiAPS=new TH1D("hHe21PhiAPS"+massbin,"Helicity Phi 2pi1pi (Accepted PS)",nbinsang,-TMath::Pi(),TMath::Pi());
 ...
 TH1D* hGJ2APS=new TH1D("hGJ2APS"+massbin,"Cos Gottfried-Jackson Theta (Accepted PS)",nbinsang,-1,1); hGJ2.push_back(hGJ2Data);
 ...
 TH1D* hTYData=new TH1D("hTYData"+massbin,"Treiman-Yang Phi (DATA)",nbinsang,-TMath::Pi(),TMath::Pi());
  ...
  TH1D* hTYAPS=new TH1D("hTYAPS"+massbin,"Treiman-Yang Phi (Accepted PS)",nbinsang,-TMath::Pi(),TMath::Pi());

vector<TH1D*> hTY2;
 TH1D* hTY2MC=new TH1D("hTY2MC"+massbin,"Treiman-Yang Phi (MC)",nbinsang,-TMath::Pi(),TMath::Pi());
 TH1D* hTY2Data=new TH1D("hTY2Data"+massbin,"Treiman-Yang Phi (DATA)",nbinsang,-TMath::Pi(),TMath::Pi());
 hTY2.push_back(hTY2MC);
 hTY2.push_back(hTY2Data);
hTY2[0]->Sumw2();
// TH1D* hTY2APS=new TH1D("hTY2APS"+massbin,"Treiman-Yang Phi (Accepted PS)",nbinsang,-TMath::Pi(),TMath::Pi());

vector<TH1D*> hTY3;
 TH1D* hTY3MC=new TH1D("hTY3MC"+massbin,"Treiman-Yang Phi (MC)",nbinsang,-TMath::Pi(),TMath::Pi());
 TH1D* hTY3Data=new TH1D("hTY3Data"+massbin,"Treiman-Yang Phi (DATA)",nbinsang,-TMath::Pi(),TMath::Pi());
...
//Loop both over data and mc tree.
 for(unsigned int itree=0;itree<2;++itree){
   ...
   for(unsigned int i=0;i<nevt;++i){
	 ...
	 if(itree==0){
	   hGJ2APS->Fill(state.p().CosTheta());
	   hMIsobar2APS->Fill(state.p().M());
	   for(unsigned int isamples=0;isamples<nsamples;++isamples){
	 ...
	 for(unsigned int js=0;js<nstates;++js){
	   ...
	   if(substate.n()==2 && substate.q()==0){
	     ...
	     if(substate.isSubstate(&state)){
	       ...
	       if(itree==0){ // mc-tree
		 hHe22APS->Fill(phe.CosTheta());
		 hHe22PhiAPS->Fill(phe.Phi());
		 for(unsigned int isamples=0;isamples<nsamples;++isamples){
	     ...
	     if(substate.isSubstate(&state)){
	       TLorentzVector phe=event.getHelicityFrame(pX,state.p(),substate.p());
	       if(itree==0){ // mc-tree
		 hHe31APS->Fill(phe.CosTheta());
		 hHe31PhiAPS->Fill(phe.Phi());
		 for(unsigned int isamples=0;isamples<nsamples;++isamples){
	 ...
	 for(unsigned int js=0;js<nstates;++js){
	   ...
	   if(substate.n()==2 && substate.q()==0){
	     ...
	     if(substate.isSubstate(&state)){
	       TLorentzVector phe=event.getHelicityFrame(pX,state.p(),substate.p());
	       if(itree==0){ // mc-tree
		 hHe21APS->Fill(phe.CosTheta());
		 hHe21PhiAPS->Fill(phe.Phi());
		 for(unsigned int isamples=0;isamples<nsamples;++isamples){
