...
void
partialWaveWeight::addWave(const std::string& keyfilename,
                           productionAmp* amp,
                           ...
                           unsigned int vectori)
{
	...
	if(keyfilename.find("+-",7) != string::npos) {
		...
		//string select;
		//cin >> select;
		if(keyfilename.find("f11285",7) != string::npos) {
			m_relphase[keyfilename] = -1;
		} else {
			m_relphase[keyfilename] = 1;
		}
...
void
partialWaveWeight::loadIntegrals(const std::string& normIntFileName, double mass) {
...
std::complex<double>
partialWaveWeight::prodAmp(unsigned int iv,
                           unsigned int iw,
                           event& e)
{
	// get mass
...
double
partialWaveWeight::weight(event& e) {
