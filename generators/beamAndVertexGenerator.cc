...
namespace {
	...
	TVectorD
	multivariateGauss(const TVectorD&    mean,
	                  const TMatrixDSym& cov,
	                  const double       sigmaScalingFactor = 1.0);
...
beamAndVertexGenerator::beamAndVertexGenerator()
	: _beamFileName(""),
	  _vertex(TVector3(0.0, 0.0, 0.0)),
	  _beam(TLorentzVector(0.0, 0.0, 0.0, 0.0)),
...
bool
beamAndVertexGenerator::loadBeamFile(const string& beamFileName)
{
...
bool
beamAndVertexGenerator::loadBeamFileWithMomenta()
{
	...
	if (not (vertexXPosition and vertexYPosition
	         and beamMomentumX and beamMomentumY and beamMomentumZ)) {
	...
	if (not (vertexXPositionSigma and vertexYPositionSigma
	         and beamMomentumXSigma and beamMomentumYSigma and beamMomentumZSigma)) {
		printWarn << "One of the optional branches for smearing the generated events with Gaussian uncertainties is missing in tree '"
		          << _beamTree->GetName() << "' in root file '" << _beamFileName << ". The branches required for smearing are:"
...
bool
beamAndVertexGenerator::loadBeamFileWithInclinations()
{
	TBranch* vertexXPosition = _beamTree->FindBranch("vertex_x_position");
	TBranch* vertexYPosition = _beamTree->FindBranch("vertex_y_position");
	TBranch* vertexZPosition = _beamTree->FindBranch("vertex_z_position");
	...
	if (not (vertexXPosition and vertexYPosition and vertexZPosition
	         and beamdXdZ and beamdYdZ and beamMomentum and vertexCov and beamMomentumSigma)) {
		printErr << "One of the required branches is missing in tree '" << _beamTree->GetName()
		         << "' in root file '" << _beamFileName << ". Required are "
...
void
beamAndVertexGenerator::randomizeBeamfileStartingPosition()
{
...
bool
beamAndVertexGenerator::check() const
{
	if ( not (_beamTree or _simulationMode == simpleSimulation))
		return false;
...
bool
beamAndVertexGenerator::event(const Target& target,
                              const Beam&   beam)
{
...
bool
beamAndVertexGenerator::eventSimple(const Target& target,
                                    const Beam&   beam)
{
	...
	_vertex.SetXYZ(target.position.X() + x, target.position.Y() + y, z);
...
bool
beamAndVertexGenerator::eventFromMomenta(const Target& target,
                                         const Beam&   beam)
{
	if (not loadNextEventFromBeamfile())
		return false;
	...
	TRandom3* randomGen = randomNumberGenerator::instance()->getGenerator();
	if (_sigmasPresent and _sigmaScalingFactor != 0.0) {
...
bool
beamAndVertexGenerator::eventFromMomentaCorrectMomentumResolution(const Target& target,
                                                                  const Beam&   beam)
{
	if (not loadNextEventFromBeamfile())
		return false;
...
bool
beamAndVertexGenerator::eventFromInclinations(const Target& target,
                                              const Beam&   beam)
{
	if (not loadNextEventFromBeamfile())
		return false;
...
bool
beamAndVertexGenerator::loadNextEventFromBeamfile()
{
...
double
beamAndVertexGenerator::getVertexZ(const Target& target) const
{
...
void
beamAndVertexGenerator::setMomentumResolutionCorrection(const double resolution,
                                                        const double momentumPDFsigma,
                                                        const double momentumPDFmean)
{
...
ostream&
beamAndVertexGenerator::print(ostream& out) const
{

	out << "Beam package information:" << endl
	    << "    Using beam file ................ " << yesNo(_simulationMode == simpleSimulation) << endl
	    << "    Beam file path ................. " << _beamFileName << endl
	    << "    Beam file loaded ............... " << yesNo(_rootFile) << endl
	    << "    Beam tree loaded ............... " << yesNo(_beamTree) << endl
	    << "    Simulation mode ................ ";
	if (_simulationMode == simulationFromMomenta) {
		...
		out << "Using momenta" << endl
		    << "    Correction for beam resolution: res = "
		    << _momentumResolution << ", PDFsigma = " << _momentumPDFSigma
		    << ", PDFmean= " << _momentumPDFMean << endl;
	} else {
		out << "Using beam inclination" << endl;
	}
	out << "    Sigmas found ................... " << yesNo(_sigmasPresent) << endl
	    << "    Sigma scaling factor ........... " << _sigmaScalingFactor << endl
	    << "    Z-positions from beam file ..... " << yesNo(_takeZpositionFromData) << endl
	    << "    Read beam file sequentially .... " << yesNo(_readBeamfileSequentially) << endl;
	if (_readBeamfileSequentially) {
...
namespace {
	...
	multivariateGauss(const TVectorD&    mean,
	                  const TMatrixDSym& cov,
	                  const double       sigmaScalingFactor)
	{
