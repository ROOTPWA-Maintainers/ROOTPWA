...
diffractivePhaseSpace::event()
{
	...
	_beam.particle.setLzVecs(make_vector_1(LorentzVector(_beamAndVertexGenerator->getBeam())));
	...
	do {
		...
		_target.recoilParticle.setLzVecs(make_vector_1(LorentzVector((beamLorentzVector + targetLab) - xSystemLab))); // targetLab
	...
	for(unsigned int i = 0; i < daughters.size(); ++i) {
		_decayProducts[i].setLzVecs(make_vector_1(LorentzVector(daughters[i])));
	}
