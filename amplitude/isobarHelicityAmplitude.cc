...
isobarHelicityAmplitude::transformDaughters()
{
  ...
  const TLorentzVector&  XLv     = _decay->xIsobarDecayVertex().inParticles()[0]->lzVec();
  const TLorentzRotation gjTrans = gjTransform(beamLv, XLv);
  for (unsigned int i = 0; i < _decay->nmbVertices(); ++i) {
    isobarDecayVertex& vertex  = *_decay->isobarDecayVertices()[i];
    if (_debug)
      printInfo << "transforming outgoing particles of vertex  "
		<< vertex.mother().name()    << "  --->  "
		<< vertex.daughter1().name() << "  +  " << vertex.daughter2().name() << "  "
  ...
  for (unsigned int i = 1; i < _decay->nmbVertices(); ++i) {  // exclude X-decay vertex
    isobarDecayVertex& vertex  = *_decay->isobarDecayVertices()[i];
    if (_debug)
      printInfo << "transforming all child particles of vertex  "
		<< vertex.mother().name()    << "  --->  "
		<< vertex.daughter1().name() << "  +  " << vertex.daughter2().name() << "  "
    ...
    decayTopology::decayGraph subGraph = _decay->subGraph(vertex);
    for (decayTopology::edgeIterator iEdge = edges(subGraph).first;
	 iEdge != edges(subGraph).second; ++iEdge) {
      particle* p = get(edge_particlePointer, subGraph, *iEdge);
      if (_debug)
...
complex<double>
isobarHelicityAmplitude::twoBodyDecayAmplitude(const isobarDecayVertex& vertex,
					       const bool               topVertex) const
{
  const particle& mother    = vertex.mother();
  const particle& daughter1 = vertex.daughter1();
  const particle& daughter2 = vertex.daughter2();
  ostringstream   debug;
  ...
  if (_debug)
    printInfo << "calculating two-body decay amplitude for " 
	      << mother.name() << "  --->  " << daughter1.name()
	      << "  +  " << daughter2.name() << ": " << debug.str()
	      << " = " << amp << endl;
  return amp;
}
...
//     of the amplitude
complex<double>
isobarHelicityAmplitude::twoBodyDecayAmplitudeSum(isobarDecayVertex& vertex,
						  const bool         topVertex)
{
  particle&       daughter1 = vertex.daughter1();
  particle&       daughter2 = vertex.daughter2();
  complex<double> ampSum    = 0;
  for (int lambda1 = -daughter1.J(); lambda1 <= +daughter1.J(); lambda1 += 2) {
    // calculate decay amplitude for daughter 1
    isobarDecayVertex* daughter1Vertex = static_cast<isobarDecayVertex*>(_decay->vertex(daughter1));
    ...
    for (int lambda2 = -daughter2.J(); lambda2 <= +daughter2.J(); lambda2 += 2) {
      ...
      isobarDecayVertex* daughter2Vertex = static_cast<isobarDecayVertex*>(_decay->vertex(daughter2));
	...
	amp2 *= twoBodyDecayAmplitudeSum(*daughter2Vertex, false);
      ...
      if (_debug)
	printInfo << "amplitude for " << vertex.mother().name()
