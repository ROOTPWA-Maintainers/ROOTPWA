                                       ...
                                       const interactionVertexPtr& prodVert)
{
	if (dynamic_pointer_cast<diffractiveDissVertex>(prodVert)) {
		...
		printWarn << "writing of keys for production vertex of this type is not yet implemented" << endl;
	  return false;
	}
}
...
bool
keyFileParser::setXDecayKeys(Setting&                   isobarDecayKey,
                             ...
                             const isobarDecayVertex&   vert)
{
	...
	bool success = true;	
	if (isobars.size() > 0) {
		Setting& isobarsKey = isobarDecayKey.add("isobars", Setting::TypeList);
	...
	isobarDecayKey.add("L", Setting::TypeInt) = (int)vert.L();
	isobarDecayKey.add("S", Setting::TypeInt) = (int)vert.S();
	if (fsParticles.size() > 0) {
		Setting& fsParticlesKey = isobarDecayKey.add("fsParticles", Setting::TypeList);
