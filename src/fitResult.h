...
namespace rpwa {
  ...
  public:
    ...
    void fill(const unsigned int                        nmbEvents,               // number of events in bin			 
	      ...
	      const std::vector<double>&                 phaseSpace);           // normalization integral over full phase space without acceptance
    ...
    inline double phaseSpace(const unsigned int waveIndexA) const;
    inline double phaseSpace(const std::string& wave) const;
    ...
    double phase       (const std::string waveA,
			const std::string waveB) const 
    {return phase(waveIndex(waveA),
		  waveIndex(waveB));}
    
    double phaseErr       (const std::string waveA,
			const std::string waveB) const 
    {return phaseErr(waveIndex(waveA),
		     waveIndex(waveB));}
  ...
  private:
    ...
    std::vector<double>                   _phaseSpace;              ///< diagonals of phase space integrals (without acceptance)
  ...
  double 
  fitResult::phaseSpace(const unsigned int waveIndexA) const
  {
    return _phaseSpace[waveIndexA];
  }
  
  double 
  fitResult::phaseSpace(const std::string& wave) const
  {
    return _phaseSpace[waveIndex(wave)];
  }
