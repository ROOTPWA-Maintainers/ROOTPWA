...
TLSContrib::TLSContrib(const TLSContrib* b, const bool& particleExchange)
	: _J(b->_J),
	  ...
	  _parameters(b->getParameters()),
	  _pureRelativistic(b->_pureRelativistic)
{
	for (size_t i = 0; i < _parameters.size(); i++) {
		if (particleExchange) {
			_parameters[i].swapExponents();
		}
	}
}
	  ...
	  _parameters(A->GetNterms())
{
	...
	for (size_t i = 0; i < _parameters.size(); i++) {
		...
		_parameters[i].squareOfPrefactor = scfac * tt.GetPreFac();
		if (signReversal == true) {
			_parameters[i].squareOfPrefactor.FlipSign();
		}
		...
		_NormFactor = _NormFactor.SumSignedRoots(_parameters[i].squareOfPrefactor);

		//    NormFactor=NormFactor+termFracNum[i]+TFracNum_Two*tSqrt;

		_parameters[i].exponentOfGammaS     = tt.GetGamS();
		_parameters[i].exponentOfGammaSigma = tt.GetGamSig();
		if (_debug) {
	...
	if (_NormFactor == TFracNum::Zero) {
		...
		for (size_t i = 0; i < GetNterms(); i++) {
			_parameters[i].squareOfPrefactor *= NormInv; // InvAbs
		}
	}
}
...
void TLSContrib::Add(const TLSContrib& rhs, bool particleExchange) {
	...
	for (size_t i = 0; i < _parameters.size(); i++) {
		_parameters[i].squareOfPrefactor *= TFracNum::Quarter * _NormFactor;
	}
			...
			if (not termSummed and _cNum == rhs._cNum                                            and
			    (
			      (particleExchange                                                              and
			       _parameters[i].exponentOfGammaS     == rhs.getParameters()[ib].exponentOfGammaSigma and
			       _parameters[i].exponentOfGammaSigma == rhs.getParameters()[ib].exponentOfGammaS
			      )                                                                              or
			      (
			       not particleExchange                                                          and
			       _parameters[i].exponentOfGammaS     == rhs.getParameters()[ib].exponentOfGammaS     and
			       _parameters[i].exponentOfGammaSigma == rhs.getParameters()[ib].exponentOfGammaSigma
			      )
			    ) )
			{
				termSummed = true;
				TFracNum bterm = TFracNum::Quarter * rhs._NormFactor * rhs.getParameters()[ib].squareOfPrefactor;

				if (_J % 2) {
					bterm.FlipSign();
				}

				_parameters[i].squareOfPrefactor = bterm.SumSignedRoots(_parameters[i].squareOfPrefactor);

			}
		}
		if (not termSummed) {

			termParameters factor(rhs.getParameters()[ib]);
	...
	//
	for (size_t i = _parameters.size(); i > 0; --i) {
		if (_parameters[i-1].squareOfPrefactor == TFracNum::Zero) {
			_parameters.erase(_parameters.begin() + (i-1));
		}
	}
	...
	if (_NormFactor == TFracNum::Zero) {
		...
		NormInv.Invert();
		for (size_t i = 0; i < _parameters.size(); i++) {
			_parameters[i].squareOfPrefactor *= NormInv;
		}
	}
}
...
void TLSContrib::Print() const
{
	...
	for (size_t iT = 0; iT < _parameters.size(); iT++) {
		const termParameters& factor = _parameters[iT];
