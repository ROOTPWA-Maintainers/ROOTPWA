...
particle::setupFrames(const int debug)
{
  ...
  if (_decay) {
    if (debug) {
      cout << "put " << Name() << Charge() << "[" << Index () << "]" << " into helicity frame:" << endl
	   << "momentum: ";
      _p.print ();
    }
		   ...
		   const int               debug)
{
  ...
  if (debug) {
    ...
    cout << "decay mass: " << _mass << endl
	 << "decay analyzer: " << child->Name() << child->Charge() << "[" << child->Index() << "]" << endl
  ...
  while (lambda1 != helicities1.end()) {
	...
	if (debug) {
	  ...
	  cout << "tilde(" << _l << "){=" << tildeFactor << "}"
	       ...
	       << "F_" << _l << "(" << ~(analyzer.get3P()) << "){=" << barrierFactor << "}"
	       << "sqrt(lambda)" << "{=" << lambdaFactor << "}"
	       << "Delta(" << _mass << ")"
	       << endl;
	}
