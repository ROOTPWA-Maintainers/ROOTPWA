...
rotation
rotation::set(double alpha, double beta, double gamma)
{
...
threeVec
operator *= (threeVec& V, const rotation& R)
{
  V = R * V;
  return V;
}
...
lorentzTransform
lorentzTransform::set(const rotation& r)
{
  this->zero();
...
lorentzTransform
lorentzTransform::set(const fourVec& p)
{
  threeVec beta;
...
fourVec
operator *= (fourVec& v, const lorentzTransform& L)
{  //new
  v = L * v;
  return v;
}
