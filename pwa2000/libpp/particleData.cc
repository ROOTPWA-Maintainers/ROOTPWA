...
void
particleData::_init(const string& nm,
		    double        m,
		    double        w,
		    int           i,
		    int           g,
...
particleData::particleData()
{
  ...
  if (_particle_data_debug)
    cerr << "in particleData(" << this << ")::particleData()\t_particle_data_count = "
...
particleData::particleData(const particleData& p)
{
  _particle_data_count++;
  if (_particle_data_debug)
    cerr << "in particleData(" << this << ")::particleData(const particleData& p="
	 << p.Name() << ")\t_particle_data_count = " << _particle_data_count << endl;
  _init(p._name, p._mass, p._width, p._isospin, p._gparity, p._spin, p._parity, p._cparity);
}


particleData::particleData(const string& n,
			   double        m,
			   double        w,
			   int           i,
			   int           g,
...
{
  if (_particle_data_debug)
    cerr << "in particleData(" << this << ")::operator=(const particleData&("
	 << &p << ")=" << p.Name() << ")\t_particle_data_count = " << _particle_data_count << endl;
  _init(p._name, p._mass, p._width, p._isospin, p._gparity, p._spin, p._parity, p._cparity);
  return(*this);
}
...
particleData&
particleData::setName(const string& nm)
{
  this->_name = nm;
  return(*this);
}
...
void
particleData::print() const
{
  ptab();
  cout << this->_name << ":\tmass=" << this->_mass << "\twidth=" << this->_width
       << "\t" << this->_isospin << getsign(this->_gparity)
       << "(" << this->_spin << getsign(this->_parity) << getsign(this->_cparity) << ")"
       << endl;
}


void
particleData::dump() const
{
  cout << this->_name    << "\t"
       << this->_mass    << "\t"
       << this->_width   << "\t"
...
void
tableEntry::_init(const particleData& p,
		  tableEntry*         n)
{
  particle     = p; 
  nextparticle = n;
...
tableEntry::tableEntry(const particleData& p,
		       tableEntry*         n)
{
  _init(p, n);
}
